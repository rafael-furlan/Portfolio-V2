...
function HeaderButton1() {
  ...
  const handleClick = () => {
    window.open('https://www.linkedin.com/in/rafaelfurlan98/', '_blank'); 
  };

  useEffect(() => {
    const sections = document.querySelectorAll('#main, #grid');
  ...
  const getIconColor = () => {
    switch (activeSection) {
     ...
     case 'm':
            return 'fill-gray-800';
      case 'grid':
        return 'fill-gray-50'; // só para testar se muda
      
    }
  };

  return (
    <div onClick={handleClick} className="flex items-center justify-center min-w-[50px] max-w-[50px] min-h-[50px] max-h-[50px] sm:min-w-[70px] sm:max-w-[70px] sm:min-h-[70px] sm:max-h-[70px] rounded-full bg-gray-600/20 backdrop-blur-md hover:bg-gray-600/40 active:bg-gray-600/60 transition-all duration-300">
        <svg
            width="20"
            height="20"
            viewBox="0 0 16 16"
            fill="currentColor" // <== permita que o Tailwind controle via fill-gray-800 etc
            xmlns="http://www.w3.org/2000/svg"
            className={`w-[15px] h-[15px] sm:w-[20px] sm:h-[20px] transition-colors duration-300 ${getIconColor()}`}
        >
